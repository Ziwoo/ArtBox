$(document).ready(function(){
	...
	//댓글 목록
	function selectData(pageNum,no_num){
		currentPage = pageNum;
		...
		$.ajax({
			type:'post',
			data:{pageNum:pageNum,no_num:no_num},
			...
			success:function(data){ 
				...
				if(count < 0 || list == null){
					...
					$(list).each(function(index,item){
						...
						output += '<div class="item" id="i'+item.re_no+'">';
						...
						if($('#userId').val() && $('#userId').val()==item.mem_id){
							output += ' <input type="button" value="수정" data-num="'+item.re_no+'" data-id="'+item.mem_id+'" class="modify_button">';
							output += ' <input type="button" value="삭제" data-num="'+item.re_no+'" data-id="'+item.mem_id+'" class="delete_button">';
						}else{
	...
	$('#re_content,#mre_content').keyup(function(){
		...
		if(inputLength>300){
			...
			if($(this).attr('mem_id')=='re_content'){
				//등록폼 글자수
